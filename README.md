...
- **Recetas de arreglo (diagnóstico opt-in, no es un gate):** los gates dicen *qué* falló, no *qué hacer*. `python scripts/rule_hints.py <RULE_ID>` da la receta de cualquiera de los 106 rule-ids que emiten los validadores, y `python scripts/preflight.py --agent` corre el preflight con la receta de cada gate en rojo ya adjunta (ver [`knowledge/validacion.md`](knowledge/validacion.md)).
