...
- **Preflight (diagnóstico opt-in, no es un gate):** antes de delegar trabajo a un agente, corre `python scripts/preflight.py` para ver en una pasada local cuáles de los 14 gates (los 13 de Nivel 1 + `validate_attestation`, local-only) fallarían sobre el repo actual; modo `--contract <nombre>` acota a 3 chequeos de un task contract (ver [`knowledge/validacion.md`](knowledge/validacion.md)).
