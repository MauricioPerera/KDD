#!/usr/bin/env python3
"""Preflight: dry-run local de los 14 gates de KDD (Contrato: preflight).

Diagnostico opt-in (NO es un gate de CI, misma familia que
``benchmark_gates.py``). Dos modos:

  Modo full (``contract is None``): corre los 14 gates (los 13 de Nivel 1 +
...
def run_preflight(repo_root='.', contract=None, runner=None, agent=False):
    ...
    Modo full (``contract is None``): los 14 gates via ``runner`` (default:
