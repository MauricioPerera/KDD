#!/usr/bin/env python3
"""Preflight: dry-run local de los 20 gates de KDD (Contrato: preflight).

Diagnostico opt-in (NO es un gate de CI, misma familia que
``benchmark_gates.py``). Dos modos:

  Modo full (``contract is None``): corre los 20 gates (los 19 de Nivel 1 +
...
def run_preflight(repo_root='.', contract=None, runner=None, agent=False):
    ...
    Modo full (``contract is None``): los 20 gates via ``runner`` (default:
