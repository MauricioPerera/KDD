...
cero drift entre el CLI y la tool MCP.
  ...
  API:
    ...
    ``run_all_level1(repo_root='.') -> {'overall_ok': bool, 'results':
      ...
      ORDEN, los 19 gates de Nivel 1 (todas las claves de ``GATE_SPECS``
...
GATE_SPECS = {
    ...
    'validate_behavior': {
        'script': 'scripts/validate_behavior.py',
        'params': ['dir'],
        'defaults': {'dir': 'behavior'},
    },
    'validate_changelog': {
...
def run_all_level1(repo_root='.'):
    """Corre los 19 gates de Nivel 1 con sus defaults contra ``repo_root``.
