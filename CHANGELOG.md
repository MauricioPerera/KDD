...
- Wired in as the 9th level-1 gate: `scripts/benchmark_gates.py`'s `GATES` tuple now measures it too, its frozen oracle (`tests/test_benchmark_gates.py`) re-sealed (`tests_sha256` updated in `knowledge/contracts/benchmark-gates.md`) to expect 9 gates instead of 8, and `.github/workflows/validate.yml` runs `validate_diagrams.py` as a real CI step. `benchmark_gates.py` itself stays out of CI on purpose (its own contract forbids that — it's a maintenance diagnostic, not a correctness gate; unrelated to whether the gates it measures are wired in). Live-verified: `benchmark_gates.py --reps 1 --warmup 0 --suite-passes 1` measures all 9 gates with exit 0, full suite 478/478 green.
