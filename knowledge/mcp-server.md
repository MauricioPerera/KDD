...
description: 'Como instalar y registrar scripts/mcp_server.py, que expone los 20 gates + orquestacion + sellado como tools MCP. Herramienta opt-in (depende del paquete externo mcp), no parte de Nivel 1/CI.'
...
El dispatch tiene 20 gates (19 de Nivel 1 y atestacion local). El wiring ofrece
13 validadores individuales y tres herramientas de orquestacion/utilidad;
los siete validadores de findings se ejecutan mediante `run_all_level1`.
Los defaults se documentan en [validacion.md](./validacion.md).

- `validate_contracts`, `validate_specs`, `validate_okf`, `lint_ascii`,
  `validate_rules`, `validate_skills`, `validate_behavior`, `validate_changelog`,
  `validate_ux_page`, `validate_diagrams`, `validate_test_commands`,
  `scan_secrets`, `validate_attestation` — un wrapper 1:1 por gate.
- `run_all_level1` — corre los 19 gates de Nivel 1 (todos excepto
  ...
  conocer los 19 gates por separado.
...
oraculo congelado del preflight (20 gates exactos).
...
|---|---|---|
| los 20 gates, `run_all_level1` | veredicto (exit 0/1) | si |
...
- **`run_all_level1` (tool MCP)** — corre los **19 gates de Nivel 1**
...
- **`scripts/preflight.py` (CLI)** — cero dependencias (stdlib + modulos
  hermanos, sin el SDK `mcp`); corre los **20 gates** (los 19 de Nivel 1
