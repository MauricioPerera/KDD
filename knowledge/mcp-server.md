...
description: 'Como instalar y registrar scripts/mcp_server.py, que expone los 18 gates + orquestacion + sellado como tools MCP. Herramienta opt-in (depende del paquete externo mcp), no parte de Nivel 1/CI.'
...
- `run_all_level1` — corre los 17 gates de Nivel 1 (todos excepto
  ...
  conocer los 17 gates por separado.
...
oraculo congelado del preflight (18 gates exactos).
...
|---|---|---|
| los 18 gates, `run_all_level1` | veredicto (exit 0/1) | si |
...
- **`run_all_level1` (tool MCP)** — corre los **17 gates de Nivel 1**
...
- **`scripts/preflight.py` (CLI)** — cero dependencias (stdlib + modulos
  hermanos, sin el SDK `mcp`); corre los **18 gates** (los 17 de Nivel 1
