...
description: 'Como instalar y registrar scripts/mcp_server.py, que expone los 17 gates + orquestacion + sellado como tools MCP. Herramienta opt-in (depende del paquete externo mcp), no parte de Nivel 1/CI.'
...
- `run_all_level1` — corre los 16 gates de Nivel 1 (todos excepto
  ...
  conocer los 16 gates por separado.
...
oraculo congelado del preflight (17 gates exactos).
...
|---|---|---|
| los 17 gates, `run_all_level1` | veredicto (exit 0/1) | si |
...
- **`run_all_level1` (tool MCP)** — corre los **16 gates de Nivel 1**
...
- **`scripts/preflight.py` (CLI)** — cero dependencias (stdlib + modulos
  hermanos, sin el SDK `mcp`); corre los **17 gates** (los 16 de Nivel 1
