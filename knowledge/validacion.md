...
- `python scripts/scan_secrets.py <dir1> [<dir2> ...]` (default `src tests`) — escaneo determinista (regex stdlib) de credenciales filtradas por prefijo de proveedor conocido (AWS `AKIA...`, GitHub `ghp_`/`gho_`/`ghu_`/`ghs_`/`ghr_`, Slack `xox[baprs]-`, Google `AIza...`, Stripe `sk_live_`/`pk_live_`) y bloques `-----BEGIN ... PRIVATE KEY-----`. Deliberadamente SIN deteccion de alta entropia generica (rompería contra los `tests_sha256` de 64 hex chars que ya viven en este repo). En el CI de ESTE repo corre como `python scripts/scan_secrets.py src` (no el default `src tests`): `tests/test_scan_secrets.py`, el oraculo del propio gate, contiene fixtures con la FORMA exacta de los patrones y se auto-detectaria como leak si se escaneara. Ver [secret-scan-gate](./contracts/secret-scan-gate.md).
