---
type: 'Task Contract'
title: 'Preflight: dry-run local de los 16 gates'
description: 'CLI de diagnostico que corre los 16 gates de KDD (los 15 de Nivel 1 + validate_attestation, el local-only que CI nunca ve) contra el repo actual y reporta cuales fallarian ANTES de que un agente toque el codigo. Modo --contract: 3 chequeos acotados a un solo task contract (frontmatter, seal del oraculo, test_command). Nace de feedback externo real de adopcion. Misma familia que benchmark_gates.py: herramienta de mantenimiento, NO gate de CI.'
...
tests_sha256: "e7973b71dc4df4fe3945f5acd54ec366c255fd50c6411e4b4ccf103f0b3a53d8"
touch_only: ['scripts/preflight.py']
deps_allowed: []
forbids: ['network', 'llm']
---

# Contract: Preflight (dry-run de los 16 gates)
...
16 gates pueden correr juntos (CI corre 15; `validate_attestation` es
...
- `ALL_GATES`: lista = `mcp_gate_dispatch.LEVEL1_GATES` +
  `['validate_attestation']` (16 nombres, en ese orden). Derivada del
...
- `run_preflight(repo_root='.', contract=None, runner=None) -> dict`:
  - Modo full (`contract is None`): corre los 16 gates de `ALL_GATES` en
  ...
  - Modo contract (`contract='<task>'`): NO corre los 16 gates. Corre 3
...
- `lines`: una linea por gate/chequeo conteniendo el nombre y su estado
  ...
  linea resumen con `<pasados>/<total>` (`16/16`, `15/16`, `3/3`, ...).
...
## Examples
- `python scripts/preflight.py` -> tabla de 16 lineas + resumen `16/16`,
  exit 0 (repo verde).
...
## Do / Don't
- DO derivar los 16 nombres del dispatch (`LEVEL1_GATES` +
...
params, repo_root, PASS/FAIL/TIMEOUT, resumen `N/16`, exit codes de
