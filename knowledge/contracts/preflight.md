---
type: 'Task Contract'
title: 'Preflight: dry-run local de los 20 gates'
description: 'CLI de diagnostico que corre los 20 gates de KDD (los 19 de Nivel 1 + validate_attestation, el local-only que CI nunca ve) contra el repo actual y reporta cuales fallarian ANTES de que un agente toque el codigo. Modo --contract: 3 chequeos acotados a un solo task contract (frontmatter, seal del oraculo, test_command). Nace de feedback externo real de adopcion. Misma familia que benchmark_gates.py: herramienta de mantenimiento, NO gate de CI.'
...
# Contract: Preflight (dry-run de los 20 gates)
...
20 gates pueden correr juntos (CI corre 19; `validate_attestation` es
...
- `ALL_GATES`: lista = `mcp_gate_dispatch.LEVEL1_GATES` +
  `['validate_attestation']` (20 nombres, en ese orden). Derivada del
...
- `run_preflight(repo_root='.', contract=None, runner=None) -> dict`:
  ...
  - Modo contract (`contract='<task>'`): NO corre los 20 gates. Corre 3
...
- `lines`: una linea por gate/chequeo conteniendo el nombre y su estado
  ...
  linea resumen con `<pasados>/<total>` (`20/20`, `19/20`, `3/3`, ...).
...
## Examples
- `python scripts/preflight.py` -> tabla de 20 lineas + resumen `20/20`,
  exit 0 (repo verde).
...
params, repo_root, PASS/FAIL/TIMEOUT, resumen `N/20`, exit codes de
