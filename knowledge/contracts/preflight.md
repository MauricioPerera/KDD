---
type: 'Task Contract'
title: 'Preflight: dry-run local de los 18 gates'
description: 'CLI de diagnostico que corre los 18 gates de KDD (los 17 de Nivel 1 + validate_attestation, el local-only que CI nunca ve) contra el repo actual y reporta cuales fallarian ANTES de que un agente toque el codigo. Modo --contract: 3 chequeos acotados a un solo task contract (frontmatter, seal del oraculo, test_command). Nace de feedback externo real de adopcion. Misma familia que benchmark_gates.py: herramienta de mantenimiento, NO gate de CI.'
...
tests_sha256: "fcdfda7caab73fbe79930e31bec7d6146a22044df473b14f45949974f22bb136"
touch_only: ['scripts/preflight.py']
deps_allowed: []
forbids: ['network', 'llm']
---

# Contract: Preflight (dry-run de los 18 gates)
...
18 gates pueden correr juntos (CI corre 17; `validate_attestation` es
...
- `ALL_GATES`: lista = `mcp_gate_dispatch.LEVEL1_GATES` +
  `['validate_attestation']` (18 nombres, en ese orden). Derivada del
...
- `run_preflight(repo_root='.', contract=None, runner=None) -> dict`:
  - Modo full (`contract is None`): corre los 18 gates de `ALL_GATES` en
...
- `lines`: una linea por gate/chequeo conteniendo el nombre y su estado
  ...
  linea resumen con `<pasados>/<total>` (`18/18`, `17/18`, `3/3`, ...).
...
## Examples
- `python scripts/preflight.py` -> tabla de 18 lineas + resumen `18/18`,
  exit 0 (repo verde).
...
## Do / Don't
- DO derivar los 18 nombres del dispatch (`LEVEL1_GATES` +
...
params, repo_root, PASS/FAIL/TIMEOUT, resumen `N/18`, exit codes de
