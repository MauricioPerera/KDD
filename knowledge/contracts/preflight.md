---
type: 'Task Contract'
title: 'Preflight: dry-run local de los 17 gates'
description: 'CLI de diagnostico que corre los 17 gates de KDD (los 16 de Nivel 1 + validate_attestation, el local-only que CI nunca ve) contra el repo actual y reporta cuales fallarian ANTES de que un agente toque el codigo. Modo --contract: 3 chequeos acotados a un solo task contract (frontmatter, seal del oraculo, test_command). Nace de feedback externo real de adopcion. Misma familia que benchmark_gates.py: herramienta de mantenimiento, NO gate de CI.'
...
tests_sha256: "1ff0ba334a5137f71aa6d6325beb47a408ef65b0d00992acce47e8e4f81ecde2"
touch_only: ['scripts/preflight.py']
deps_allowed: []
forbids: ['network', 'llm']
---

# Contract: Preflight (dry-run de los 17 gates)
...
17 gates pueden correr juntos (CI corre 16; `validate_attestation` es
...
- `ALL_GATES`: lista = `mcp_gate_dispatch.LEVEL1_GATES` +
  `['validate_attestation']` (17 nombres, en ese orden). Derivada del
...
- `run_preflight(repo_root='.', contract=None, runner=None) -> dict`:
  - Modo full (`contract is None`): corre los 17 gates de `ALL_GATES` en
  ...
  - Modo contract (`contract='<task>'`): NO corre los 17 gates. Corre 3
...
- `lines`: una linea por gate/chequeo conteniendo el nombre y su estado
  ...
  linea resumen con `<pasados>/<total>` (`17/17`, `16/17`, `3/3`, ...).
...
## Examples
- `python scripts/preflight.py` -> tabla de 17 lineas + resumen `17/17`,
  exit 0 (repo verde).
...
## Do / Don't
- DO derivar los 17 nombres del dispatch (`LEVEL1_GATES` +
...
params, repo_root, PASS/FAIL/TIMEOUT, resumen `N/17`, exit codes de
