...
fixtures en un tmpdir aislado.
  ...
  API que congela:
    ``ALL_GATES`` -- ``LEVEL1_GATES`` + ``['validate_attestation']`` (15).
    ``run_preflight(repo_root='.', contract=None, runner=None) -> dict``
      modo full (contract None): corre los 15 gates via ``runner``
      ...
      modo contract: NO corre los 15 gates; corre 3 chequeos sobre
...
class TestAllGatesConstant(unittest.TestCase):
    def test_all_gates_es_level1_mas_attestation(self):
        ...
        self.assertEqual(len(preflight.ALL_GATES), 15)
...
class TestFullMode(unittest.TestCase):
    ...
    def test_lineas_y_resumen_todos_pasan(self):
        ...
        self.assertTrue(any('15/15' in ln for ln in lines),
                        'falta resumen 15/15')
    ...
    def test_un_gate_falla(self):
        ...
        self.assertTrue(any('14/15' in ln for ln in lines))
...
class TestContractMode(unittest.TestCase):
    ...
    def test_modo_contract_no_corre_los_15_gates(self):
        tmp = self._make_repo()
