...
fixtures en un tmpdir aislado.
  ...
  API que congela:
    ``ALL_GATES`` -- ``LEVEL1_GATES`` + ``['validate_attestation']`` (20).
    ``run_preflight(repo_root='.', contract=None, runner=None) -> dict``
      modo full (contract None): corre los 20 gates via ``runner``
      ...
      modo contract: NO corre los 20 gates; corre 3 chequeos sobre
...
class TestAllGatesConstant(unittest.TestCase):
    def test_all_gates_es_level1_mas_attestation(self):
        ...
        self.assertEqual(len(preflight.ALL_GATES), 20)
...
class TestFullMode(unittest.TestCase):
    ...
    def test_lineas_y_resumen_todos_pasan(self):
        ...
        self.assertTrue(any('20/20' in ln for ln in lines),
                        'falta resumen 20/20')
    ...
    def test_un_gate_falla(self):
        ...
        self.assertTrue(any('19/20' in ln for ln in lines))
